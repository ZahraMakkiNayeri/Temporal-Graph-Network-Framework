...
def get_data(graph_df, edge_features, node_features, different_new_nodes_between_val_and_test=False,
             randomize_features=False, val_time=None, test_time=None):
    """val_time / test_time override the default 70th/85th-percentile boundaries.
    Used by the time-window experiment, which splits on calendar days instead."""
    #balanced_reindexed_df, feature_array, node_features = run(graph_df)

    if randomize_features:
        node_features = np.random.rand(node_features.shape[0], node_features.shape[1])

    if val_time is None or test_time is None:
        val_time, test_time = list(np.quantile(graph_df.ts, [0.70, 0.85]))
    else:
        print(f'split boundaries supplied explicitly: val_time={val_time:.1f}, '
              f'test_time={test_time:.1f}')
    ...
    # Check for out-of-bound indices in edge_idxs
    # Check for out-of-bound indices in edge_idxs
    ...
    # Sample nodes which we keep as new nodes (to test inductiveness), so we have to remove all their edges from training
...
#############################################################
# Example definition of args
#args = {
#    'different_new_nodes': False,  # or True, depending on your requirements
#    'randomize_features': False,   # or True, depending on your requirements
#    # Add other necessary arguments
#}
#
## Extract data for training, validation and testing
#node_features, edge_features, full_data, train_data, val_data, test_data, new_node_val_data, \
#new_node_test_data = get_data(preprocessed_df, edge_features, node_features,
